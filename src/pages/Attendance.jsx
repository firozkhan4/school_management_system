...
import {
  ...
  BookOpen, ToggleLeft, ToggleRight, ArrowDown, ArrowUp
} from 'lucide-react';
...
const initialStudents = [
  ...
  { id: 105, name: "Kiran Rao", status: false, roll: 5 },
];
...
const initialTeachers = [
  ...
  { id: 204, name: "Mr. Rajan Kohli (Admin Head)", status: true, department: 'Administration' },
];

// Reusable Button Component for Attendance Toggle
...
const Attendance = () => {
  ...
  // Handler to toggle attendance status for an individual
  ...
  // Handler to mark all present/absent
  ...
  // Handler for saving the attendance record
  const handleSave = () => {
    setLoading(true);
    setMessage('');

    const attendanceData = activeTab === 'student' ? studentList : teacherList;
    const saveTarget = activeTab === 'student'
      ? `Class ${currentClass} - Section ${currentSection}`
      : `Department ${currentDepartment}`;

    console.log(`Saving Attendance for ${activeTab}s on ${currentDate}. Target: ${saveTarget}`, attendanceData);
    ...
    if (list.length === 0) {
      return (
        <div className="text-center p-8 text-gray-500 bg-gray-50 rounded-lg">
          No {isStudent ? 'students' : 'teachers'} found for this criteria.
        </div>
      );
    }
      ...
      <div className="shadow-lg rounded-xl overflow-hidden bg-white">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200">
          ...
          <div className="hidden sm:flex space-x-2">
            <button
              onClick={() => setAllAttendance(listType, true)}
              className="px-3 py-1 text-xs font-medium rounded-lg text-green-700 bg-green-100 hover:bg-green-200 transition"
            >
              Mark All Present
            </button>
            <button
              onClick={() => setAllAttendance(listType, false)}
              className="px-3 py-1 text-xs font-medium rounded-lg text-red-700 bg-red-100 hover:bg-red-200 transition"
            >
              Mark All Absent
            </button>
          </div>
    ...
    >
      ...
      <header className="border-b border-gray-300 pb-4 mb-6">
        <h1 className="text-3xl font-extrabold text-gray-900 flex items-center">
          <Clock size={32} className="mr-3 text-blue-600" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {message && (
            <div className={`p-4 rounded-lg text-sm transition-opacity duration-300 ${message.includes('successfully') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {message}
            </div>
          )}
